...
Welcome to Rubiks 3D.

Note: You must have NumPy and MatPlotLib libraries 
installed in your device to run this code since its 
dependant on these libraries.

Arrays are used to represent the faces
...
Each face has the face number appended
at the end in order to sort correctly using lexsort.

This number is the centroid and is equal to sum_i[vi].

Basically, there are 3 arrays we initialize and use to store the state of the cube:
  centroids_store = (6 * N * N, 4)
  faces_store = (6 * N * N, 5, 3)
  stickers_store = (6 * N * N, 9, 3)
  colors_store = (6 * N * N,)

The canonical order is found by doing
  ind = np.lexsort(centroids.T)

Credit: Inspired by David W. Hogg's rotations handler to handle i/o of mouse.
"""
...
from rotations import Rotation_vector_handler, project_points
...
class Cube:
    ...
    x, y, z = np.eye(3)
    rots = [Rotation_vector_handler.create_rot_v_theta(np.eye(3)[0], theta)
    for theta in (np.pi / 2, -np.pi / 2)]
    rots += [Rotation_vector_handler.create_rot_v_theta(np.eye(3)[1], theta)
    ...
    def rotate_face(self, f, n=1, layer=0):
        ...
        v = self.facesdict[f]
        r = Rotation_vector_handler.create_rot_v_theta(v, n * np.pi / 2)
...
class InteractiveCube(plt.Axes):
    def __init__(self, cube=None,
        ...
        self._view = view
        self._start_rot = Rotation_vector_handler.create_rot_v_theta((1, -1, 0),
                                                  -np.pi / 6)
    ...
    def _initialize_widgets(self):
        ...
        self._btn_reset = widgets.Button(self._ax_reset, 'Reset frame')
        ...
        self._ax_undo = self.figure.add_axes([0.35, 0.05, 0.2, 0.075])
        self._btn_undo = widgets.Button(self._ax_undo, 'Undo')
    ...
    def _undo(self, *args):
        if len(self.cube._move_list) != 0:
            last = self.cube._move_list[-1]
            (face, n, layer) = last
            self.rotate_face(face, n, layer, steps =1)
            last.pop(-1)
    ...
    def _key_press(self, event):
        ...
        if event.key == 'shift':
            ...
            if self._shift:
                ...
                ax_LR = self._ax_LR
            self.rotate(Rotation_vector_handler.create_rot_v_theta(ax_LR,
                                                5 * self._step_LR))
        elif event.key == 'left':
            if self._shift:
                ax_LR = self._ax_LR_alt
            ...
            self.rotate(Rotation_vector_handler.create_rot_v_theta(self._ax_UD,
                                                5 * self._step_UD))
        elif event.key == 'down':
            self.rotate(Rotation_vector_handler.create_rot_v_theta(self._ax_UD,
                                                -5 * self._step_UD))
        ...
        if self._button1 or self._button2:
            ...
            if self._button1:
                if self._shift:
                    ...
                    ax_LR = self._ax_LR
                rot1 = Rotation_vector_handler.create_rot_v_theta(self._ax_UD,
                                               self._step_UD * dy)
                rot2 = Rotation_vector_handler.create_rot_v_theta(ax_LR,
                                               self._step_LR * dx)
